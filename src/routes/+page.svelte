<script>
	const baseTitle = 'Black Pearl Racing Team - KMUTT';
</script>

<svelte:head>
	<title>{baseTitle}</title>
</svelte:head>

<div class="p-2">
	<p class="font-sans text-5xl font-bold">Black Pearl Racing Team</p>
	<p class="text-2xl">
		<a
			href="https://www.fsaeonline.com/"
			target="_blank"
			class="font-semibold text-amber_SAE_ECE underline hover:underline">Formula Student</a
		>
		team of
		<a
			href="https://www.kmutt.ac.th/"
			target="_blank"
			class="font-semibold text-amber_SAE_ECE underline hover:underline">KMUTT</a
		>.
	</p>

	<!-- TODO: Hero text concept -->
	<!-- We represent KMUTT's finest craftsmanship in the world's biggest engineering design competition. -->

	<!-- We invite you to join ... if you are ... -->
</div>

<!-- <div class="h-[20rem]"></div> -->
